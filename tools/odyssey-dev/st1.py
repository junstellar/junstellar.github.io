#!/usr/bin/env python
"""
오디세이아 플레이 드라이버 — shunting-dev/play.py 의 오디세이 판(경로·상태 JS 만 다름).

  python play.py --out shots/try1 --actions "shot; eval __SHOT.solve(); eval __SHOT.run(); wait 4000; shot"
...
액션:
  ...
  text                  화면 DOM 텍스트 덤프 (카드 내용 확인)
  click X,Y             화면 좌표 클릭 (CSS px)
  key K                 키 입력
  wait MS               대기 (실행 애니메이션은 2~4초)
  drag X1,Y1,X2,Y2      드래그
  wheel D               휠
  eval JS               페이지에서 JS 실행하고 결과 출력

디버그 API(90-boot.js): __SHOT.ready / stage(i) / place(id) / run() / state()
                        + solve() next() retry() clear() tap(x,y) card()
...
def main():
    ...
    ap.add_argument("--path", default="/odyssey/st1.html")
    ...
    url = a.url or f"http://127.0.0.1:{port}{a.path}{q}"
