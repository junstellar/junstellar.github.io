#!/bin/bash
# ComfyUI-short render (clone of render_v6.sh). Per-scene encode -> xfade chain
# (0.5s x5) -> narration adelay+amix + looped BGM (volume 0.15, fadein 1.5,
# fadeout st=DUR-2.53 d=2.5, amix normalize=0), mux AAC. Frames from
# seq_comfy_${LANG}_s*, narration from tts_comfy_${LANG}/scene*.mp3.
# Usage: render_comfyui.sh ko|en OUT.mp4
set -e
LANG=$1
OUT=$2
cd "$(dirname "$0")"
...
if [ "$LANG" = "ko" ]; then
  OFF=(11.56667 21.36667 30.3 37.96667 46.46667)
  ADEL=(300 11867 21667 30600 38267 46767)
  DUR=52.83333
  DFO=50.30333          # DUR - 2.53
else
  OFF=(7.6 15.0 21.83333 28.83333 35.06667)
  ADEL=(300 7900 15300 22133 29133 35367)
  DUR=39.23333
  DFO=36.70333
fi
...
for n in 1 2 3 4 5 6; do
  ffmpeg -y -v error -framerate 30 -i seq_comfy_${LANG}_s${n}/%05d.png \
    -c:v libx264 -crf 18 -preset medium -pix_fmt yuv420p -r 30 \
    scene_comfy_${LANG}_s${n}.mp4
done

echo "== xfade chain =="
ffmpeg -y -v error \
  -i scene_comfy_${LANG}_s1.mp4 -i scene_comfy_${LANG}_s2.mp4 -i scene_comfy_${LANG}_s3.mp4 \
  -i scene_comfy_${LANG}_s4.mp4 -i scene_comfy_${LANG}_s5.mp4 -i scene_comfy_${LANG}_s6.mp4 \
  -filter_complex "\
...
[d][5]xfade=transition=fade:duration=0.5:offset=${OFF[4]}[v]" \
  ...
  video_comfy_${LANG}.mp4

echo "== narration + BGM mix, mux =="
ffmpeg -y -v error \
  -i video_comfy_${LANG}.mp4 \
  -i tts_comfy_${LANG}/scene1.mp3 -i tts_comfy_${LANG}/scene2.mp3 \
  -i tts_comfy_${LANG}/scene3.mp3 -i tts_comfy_${LANG}/scene4.mp3 \
  -i tts_comfy_${LANG}/scene5.mp3 -i tts_comfy_${LANG}/scene6.mp3 \
